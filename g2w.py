...
def crop_image(im):
    img = cv2.imread(im) # Read in the image and convert to grayscale
    img = img[40:-70,0:-1] # Perform pre-cropping
    ...
    w = w + x + 50
    rect = img[y:y+h, 0:w]
    cv2.imwrite(im, rect) # Save the image
    
def create_powerpoint(name):
